...
//    self.userName=  [NSString stringWithFormat:@"user%d",value];
    NSString *username = [NSString stringWithFormat:@"user%d",value];

    [self showHudInView:self.view hint:@"正在初始化"];
    __weak typeof(self) weakself = self;
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        EMError *error = [[EMClient sharedClient] registerWithUsername:username password:@"888888"];
        dispatch_async(dispatch_get_main_queue(), ^{
            [weakself hideHud];
            if (!error) {
//                TTAlertNoTitle(NSLocalizedString(@"register.success", @"Registered successfully, please log in"));
                    [self loginWithUsername:username password:@"888888"];
            }else{
                switch (error.code) {
